layui.config({
    ...
    function linksList(that) {
        ...
        function renderDate(data, curr) {
            var dataHtml = '';
            if (!that) {
                currentData = linksData.concat().splice(curr * nums - nums, nums);
            } else {
                currentData = that.concat().splice(curr * nums - nums, nums);
            }
            if (currentData.length !== 0) {
                for (var i = 0; i < currentData.length; i++) {
                    dataHtml += '<tr>'
                        + '<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'
                        + '<td align="left">' + currentData[i].id + '</td>'
                        + '<td>' + currentData[i].name + '</td>'
                        + '<td>' + currentData[i].nickName + '</td>'
                        + '<td>' +
                        (currentData[i].keyRule === 1 ? "id" : currentData[i].keyRule === 2 ? "uuid" : "时间戳")
                        + '</td>'
                        + '<td>' + currentData[i].isSplit + '</td>'
                        + '<td>' + currentData[i].frontSection + '</td>'
                        + '<td>' + currentData[i].frontSectionRange + '</td>'
                        + '<td>' +
                        (currentData[i].frontSectionRepeatable === 1 ? "是" : "否")
                        + '</td>'
                        + '<td>' + currentData[i].backArea + '</td>'
                        + '<td>' + currentData[i].backAreaRange + '</td>'
                        + '<td>' +
                        (currentData[i].backAreaRepeatable === 1 ? "是" : "否")
                        + '</td>'
                        // + '<td>'
                        + '<td>' +
                        (currentData[i].isHandler === 1 ? "是" : "否")
                        + '</td>'
                        + '<td>'
