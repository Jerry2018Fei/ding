layui.config({
	...
	function newsList(that){
		...
		function renderDate(data,curr){
			var dataHtml = '';
			if(!that){
				currentData = newsData.concat().splice(curr*nums-nums, nums);
			}else{
			...
			if(currentData.length != 0){
				for(var i=0; i<currentData.length; i++){
			    	...
			    	+'<td align="left">'+currentData[i].newsName+'</td>'
			    	+'<td>'+currentData[i].newsAuthor+'</td>';
			    	if(currentData[i].newsStatus == "待审核"){
			    		dataHtml += '<td style="color:#f00">'+currentData[i].newsStatus+'</td>';
			    	}else{
			    		dataHtml += '<td>'+currentData[i].newsStatus+'</td>';
			    	}
			    	dataHtml += '<td>'+currentData[i].newsLook+'</td>'
			    	+'<td><input type="checkbox" name="show" lay-skin="switch" lay-text="是|否" lay-filter="isShow"'+currentData[i].isShow+'></td>'
			    	+'<td>'+currentData[i].newsTime+'</td>'
			    	+'<td>'
