layui.config({
	...
	function usersList(){
		...
		function renderDate(data,curr){
			var dataHtml = '';
			currentData = usersData.concat().splice(curr*nums-nums, nums);
			if(currentData.length != 0){
				for(var i=0; i<currentData.length; i++){
					dataHtml += '<tr>'
			    	+  '<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'
			    	+  '<td>'+currentData[i].userName+'</td>'
			    	+  '<td>'+currentData[i].userEmail+'</td>'
			    	+  '<td>'+currentData[i].userSex+'</td>'
			    	+  '<td>'+currentData[i].userGrade+'</td>'
			    	+  '<td>'+currentData[i].userStatus+'</td>'
			    	+  '<td>'+currentData[i].userEndTime+'</td>'
			    	+  '<td>'
